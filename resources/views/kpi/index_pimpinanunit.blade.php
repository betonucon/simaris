...
@push('datatable')
    <script>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
               type: 'GET',
               url: "{{url('kpi/view_data_pimpinanunit?unit='.$unit_id)}}&periode="+periode,
               data: "id=id",
               beforeSend: function(){
        ...
        function cari(a){
           ...
           $.ajax({
               type: 'GET',
               url: "{{url('kpi/view_data_pimpinanunit?unit='.$unit_id)}}&periode="+a,
               data: "id=id",
               beforeSend: function(){
                    $("#tampilkan").html('<center><img src="{{url(url_link().'/img/loading.gif')}}" width="3%"> Proses Data.............</center>');
               },
        ...
        function cek(a){
           ...
           $.ajax({
               ...
               success: function(msg){
                    $('#modalloading').modal('hide');
                    $("#tampilkan").load("{{url('kasir/view_data_pimpinanunit?unit='.$unit_id.'&periode='.$periode)}}");
        ...
        function hapus(a){
            if (confirm('Apakah yakin akan menghapus data ini?')) {
                $.ajax({
                    ...
                    success: function(msg){
                            $('#modalloading').modal('hide');
                            $("#tampilkan").load("{{url('kpi/view_data_pimpinanunit?unit='.$unit_id.'&periode='.$periode)}}");
