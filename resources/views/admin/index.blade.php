            ...
            <div class="modal-body">
                ...
                <form method="post" id="mysimpan_data" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="form-group">
                            <label>NIK </label>
                            <input type="text"  onkeyup="cari_nik(this.value)" placeholder="Cari pengguna" value="" class="form-control">
                            <input type="text"  disabled  id="nama" value="" class="form-control">
                            <input type="hidden"  name="kode"  id="kode" value="" class="form-control">
                        </div>
                        
                        <div class="form-group">
                            <label>Role</label>
                            <select  name="role_id" onchange="cek_role(this.value)"  class="form-control">
...
@push('datatable')
    <script>
        ...
        function cari(a){
           ...
           $.ajax({
               type: 'GET',
               url: "{{url('admin/view_data')}}?name="+a,
               data: "id=id",
               beforeSend: function(){
                    $("#tampilkan").html('<center><img src="{{url(url_link().'/img/loading.gif')}}" width="3%"> Proses Data.............</center>');
               },
        ...
        function hapus(a){
            if (confirm('Apakah yakin akan menghapus data ini?')) {
                $.ajax({
                    ...
                    success: function(msg){
                            $('#modalloading').modal('hide');
                            $("#tampilkan").load("{{url('admin/view_data')}}");
